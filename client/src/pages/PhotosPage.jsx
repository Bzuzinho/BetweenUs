    ...
    <div style={{ padding:'10px 12px' }}>
      ...
      <div style={{ display:'flex', gap:4 }}>
        {VISIBILITY_VALUES.map(value => {
          ...
          return <button key={value} title={item.label} onClick={() => changeVisibility(value)} disabled={saving} style={{ flex:1, background:visibility===value?C.primaryDim:C.input, border:`1px solid ${visibility===value?C.primary:C.border}`, borderRadius:8, padding:'5px 2px', cursor:'pointer', opacity:saving?.6:1 }}>{item.icon}</button>
        })}
      </div>
    </div>
  </div>
    ...
    <div style={{ maxWidth:420, margin:'0 auto' }}>
      ...
      {photos.length < 6 && <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:20, marginBottom:20 }}>
        ...
        {preview ? <>
          ...
          <button onClick={upload} disabled={uploading} style={{ width:'100%', background:C.primary, border:'none', borderRadius:50, padding:13, fontWeight:600, color:'#0A141A', opacity:uploading?.7:1 }}>{uploading?t('photos.uploading'):t('photos.upload')}</button>
        </> : <>
