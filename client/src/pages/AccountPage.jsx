...
export default function AccountPage() {
  ...
  useEffect(() => {
    Promise.allSettled([api.get('/auth/me'), api.get('/push/language')]).then(results => {
      const account = results[0].status === 'fulfilled' ? results[0].value.data : {}
      const preferredLanguage = results[1].status === 'fulfilled' ? results[1].value.data.preferredLanguage : language
      setForm({ accountName:account.accountName || '', nif:account.nif || '', preferredLanguage })
      setLanguage(preferredLanguage)
      if (account.avatarPath) setAvatarPreview(account.avatarPath)
    })
      ...
      <div className="account-page-inner" style={{ maxWidth:560, margin:'0 auto' }}>
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:24, marginBottom:14 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:20 }}>
            ...
            <div><div style={{ fontSize:14, color:C.text, marginBottom:4 }}>{t('account.accountPhoto')}</div><div style={{ fontSize:12, color:C.muted, lineHeight:1.5 }}>{t('account.accountPhotoHelp')}</div><input ref={fileRef} type="file" accept="image/*" style={{ display:'none' }} onChange={handleAvatarChange}/></div>
          </div>
        </div>
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:24, marginBottom:14 }}>
          ...
          <div style={{ ...INP, color:C.muted, cursor:'not-allowed', display:'flex', alignItems:'center', marginBottom:12 }}>{user?.email}<span style={{ marginLeft:'auto', fontSize:11, color:emailVerified ? C.success : C.danger }}>{emailVerified ? `✓ ${t('common.verified')}` : `✗ ${t('common.notVerified')}`}</span></div>
          ...
          <select style={INP} value={form.preferredLanguage} onChange={event => handleLanguageChange(event.target.value)}>{LANGUAGE_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:24, marginBottom:14 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}><div><div style={{ fontSize:15, fontWeight:500, color:C.text }}>{isPremium ? 'Between Plus' : t('account.freePlan')}</div><div style={{ fontSize:12, color:C.muted, marginTop:2 }}>{isPremium && sub?.currentPeriodEnd ? `${t('account.renewsOn')} ${formatDate(sub.currentPeriodEnd)}` : t('account.noSubscription')}</div></div>{isPremium ? <span style={{ fontSize:11, background:C.primaryDim, border:'1px solid rgba(184,167,255,0.3)', borderRadius:20, padding:'4px 12px', color:C.primary }}>✦ {t('common.active')}</span> : <button onClick={() => navigate('/premium')} style={{ background:C.primary, border:'none', borderRadius:50, padding:'8px 16px', fontSize:13, fontWeight:500, color:'#0A141A', cursor:'pointer' }}>{t('account.upgrade')}</button>}</div>
        </div>
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, padding:24, marginBottom:14 }}><div style={{ fontSize:11, color:C.muted, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>{t('account.accountType')}</div><div style={{ fontSize:15, color:C.text }}>{user?.adminRole || t('common.user')}</div></div>
        <div style={{ background:'rgba(248,113,113,0.04)', border:'1px solid rgba(248,113,113,0.15)', borderRadius:20, padding:24 }}><div style={{ fontSize:11, color:C.danger, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>{t('account.dangerZone')}</div><button onClick={() => navigate('/delete-account')} style={{ background:'none', border:'1px solid rgba(248,113,113,0.3)', borderRadius:10, padding:'10px 16px', fontSize:13, color:C.danger, cursor:'pointer' }}>{t('account.deleteAccount')}</button></div>
      </div>
    </div>
  )
}
