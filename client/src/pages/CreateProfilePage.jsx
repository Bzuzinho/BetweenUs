    ...
    <div style={{ maxWidth:420, margin:'0 auto' }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.plum}`, borderRadius:24, padding:24 }}>
        ...
        {step===4 && <>
          ...
          <div style={{ display:'flex', gap:10 }}><button style={{ ...secondary, flex:1 }} onClick={() => setStep(3)}>{t('profileForm.back')}</button><button style={{ ...primary, flex:2, opacity:loading?.7:1 }} onClick={submit} disabled={loading}>{loading?t('profileForm.creating'):t('profileForm.create')}</button></div>
        </>}
      </div>
    </div>
  </div>
