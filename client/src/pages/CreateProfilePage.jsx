    ...
    <div style={{ maxWidth:420, margin:'0 auto' }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.plum}`, borderRadius:24, padding:24 }}>
        {step===1 && <>
          ...
          <select style={inputStyle} value={form.gender} onChange={event => set('gender', event.target.value)}><option value="">{t('profileForm.gender')}</option>{catalogGenders.map(item => <option key={item.id} value={item.slug}>{catalogLabel(t, 'genders', item.slug, item.label)}</option>)}</select>
          <select style={inputStyle} value={form.orientation} onChange={event => set('orientation', event.target.value)}><option value="">{t('profileForm.orientation')}</option>{catalogOrientations.map(item => <option key={item.id} value={item.slug}>{catalogLabel(t, 'orientations', item.slug, item.label)}</option>)}</select>
        ...
        {step===4 && <>
          ...
          <div style={{ marginBottom:24, maxHeight:360, overflowY:'auto' }}>{Object.entries(catalogBoundaries.reduce((groups, boundary) => { (groups[boundary.category] ||= []).push(boundary); return groups }, {})).map(([category, items]) => <div key={category} style={{ marginBottom:14 }}><div style={{ fontSize:11, color:C.muted, textTransform:'uppercase', marginBottom:6 }}>{catalogLabel(t, 'boundaryCategories', category, category.replace(/_/g, ' '))}</div>{items.map(boundary => <div key={boundary.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, padding:'8px 0', borderBottom:`1px solid ${C.plum}` }}><span style={{ fontSize:13, color:C.white }}>{catalogLabel(t, 'boundaries', boundary.slug, boundary.name)}</span><div style={{ display:'flex', gap:5 }}>{['NO','MAYBE','YES'].map(preference => <button key={preference} type="button" onClick={() => setBoundaryPref(boundary.id, preference)} style={{ background:boundaryPrefs[boundary.id]===preference?C.accent:'transparent', border:`1px solid ${C.plum}`, borderRadius:8, padding:'4px 8px', color:boundaryPrefs[boundary.id]===preference?'#0A141A':C.muted }}>{t(`profileForm.${preference==='YES'?'yes':preference==='MAYBE'?'maybe':'no'}`)}</button>)}</div></div>)}</div>)}</div>
