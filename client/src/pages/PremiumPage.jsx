      ...
      <div style={{ maxWidth:420, margin:'0 auto' }}>
        ...
        {msg && (
          <div style={{ background: msg.startsWith('✅')
            ...
            border: `1px solid ${msg.startsWith('✅') ? colors.green : colors.accent}`,
            borderRadius:12, padding:'12px 16px', marginBottom:20,
            color: msg.startsWith('✅') ? colors.green : colors.accent,
            fontSize:13 }}>{msg}</div>
        )}
          ...
          <div style={{ background:'rgba(61,214,140,0.08)',
            ...
            <button onClick={handlePortal}
              style={{ background:'none', border:`1px solid ${colors.plum}`,
                borderRadius:50, padding:'10px 24px', color:colors.lavLight,
          ...
          <div key={plan.id} style={{ background:colors.bgCard,
            ...
            <div style={{ display:'flex', alignItems:'flex-start',
              ...
              <div>
                ...
                <div style={{ fontFamily:"'Playfair Display',serif", fontSize:20,
                  fontWeight:700, color:colors.white }}>{plan.name}</div>
              </div>
              <div style={{ textAlign:'right' }}>
                <div style={{ fontSize:28, fontWeight:700, color:plan.color }}>
                  {plan.price}
                </div>
                <div style={{ fontSize:12, color:colors.muted }}>{plan.period}</div>
              </div>
            </div>
            ...
            <div style={{ display:'flex', flexDirection:'column',
              ...
              {plan.features.map((f, i) => (
                <div key={i} style={{ fontSize:13, color:colors.lavLight,
            ...
            <button
              ...
              style={{ width:'100%',
                background: plan.id === 'PREMIUM'
                  ...
                  : `linear-gradient(135deg,${colors.lavender},${colors.lavLight})`,
                border:'none', borderRadius:50, padding:'15px',
                fontSize:15, fontWeight:700,
                color: plan.id === 'PREMIUM' ? '#1A0A2E' : '#0E0818',
