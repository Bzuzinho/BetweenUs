...
function ProfileCard({ profile, onLike, onPass }) {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (res?.match) {
        setMatchPop(true)
        setTimeout(() => setMatchPop(false), 3000)
      }
    } catch {}
  }
    ...
    <>
      <div style={{ background:colors.bgCard, border:`1px solid ${colors.plum}`,
        ...
        <div style={{ height:280, background:'linear-gradient(135deg,#3D2060,#0E0818)',
          ...
          {profile.photos?.length > 0 ? (
            <img src={profile.photos[0].storagePath} alt=""
              style={{ width:'100%', height:'100%', objectFit:'cover',
                filter: profile.photos[0].visibilityLevel === 'BLURRED'
                  ? 'blur(12px)' : 'none' }} />
          ) : (
          ...
          <div style={{ position:'absolute', top:14, left:14,
            ...
            color:colors.accentLight, fontWeight:600, backdropFilter:'blur(4px)' }}>
            ★ {profile.betweenScore}
          </div>

          {profile.photos?.length === 0 && (
            <div style={{ position:'absolute', background:'rgba(201,149,107,0.15)',
              border:`1px solid ${colors.accent}`, borderRadius:20,
              padding:'6px 14px', fontSize:11, color:colors.accentLight,
              backdropFilter:'blur(4px)' }}>📷 Sem foto ainda</div>
          )}
        </div>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={handlePass}
              style={{ flex:1, background:colors.bgInput, border:'none',
                borderRadius:14, padding:13, color:colors.muted,
                cursor:'pointer', fontSize:13, fontWeight:600,
                fontFamily:'Inter,sans-serif' }}>Passar</button>
      ...
      {matchPop && (
        <div onClick={() => setMatchPop(false)}
          ...
          <button onClick={() => setMatchPop(false)}
            style={{ background:`linear-gradient(135deg,${colors.accent},${colors.rose})`,
              border:'none', borderRadius:50, padding:'14px 36px', fontSize:15,
              fontWeight:700, color:'#1A0A2E', cursor:'pointer', marginTop:8 }}>
            Ver Matches
          </button>
        </div>
      )}
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20 }}>
        {FILTERS.map(f => (
          <div key={f.key} onClick={() => setFilter(f.key)}
            style={{ background: filter === f.key
              ? 'rgba(201,149,107,0.15)' : colors.bgCard,
      ...
      {loading && (
        <div style={{ textAlign:'center', padding:'60px 0' }}>
          <div style={{ color:colors.accent, fontFamily:"'Playfair Display',serif",
            fontSize:18, fontStyle:'italic', marginBottom:8 }}>A carregar...</div>
          <div style={{ color:colors.muted, fontSize:13 }}>
            A procurar ligações compatíveis
          </div>
        </div>
      )}
        ...
        <div style={{ background:'rgba(224,92,122,0.1)',
          ...
          <button onClick={loadProfiles}
            style={{ marginTop:12, background:'none',
              border:`1px solid #E05C7A`, borderRadius:50,
              padding:'8px 20px', color:'#E05C7A', cursor:'pointer',
              fontSize:12 }}>Tentar novamente</button>
        </div>
      )}
      ...
      {empty && !loading && !error && (
        <div style={{ textAlign:'center', padding:'60px 20px' }}>
          ...
          <div style={{ color:colors.muted, fontSize:14, lineHeight:1.6 }}>
            Ainda não há outros perfis para explorar.<br />
            Convida alguém para experimentar a plataforma.
          </div>
        </div>
      )}
