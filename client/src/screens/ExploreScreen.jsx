...
import { createExploreTranslator } from '../i18n/exploreTranslations'
...
function GridTile({ profile, onOpen, t }) {
  return <button type="button" onClick={() => onOpen(profile)} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, overflow:'hidden', cursor:'pointer', position:'relative', padding:0, textAlign:'left' }}>
    <div style={{ aspectRatio:'3 / 4', width:'100%', background:`linear-gradient(160deg, ${C.elevated}, ${C.bg})`, position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
      {profile.hasPhotos && profile.primaryPhoto ? <img src={profile.primaryPhoto} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/> : <span style={{ fontSize:22, color:C.muted }}>{TYPE_ICON[profile.type] || '○'}</span>}
      {profile.type !== 'INDIVIDUAL' && <div style={{ position:'absolute', top:6, left:6, background:'rgba(10,20,26,.85)', border:'1px solid rgba(184,167,255,.3)', borderRadius:20, padding:'2px 7px', fontSize:9, color:C.primary }}>{TYPE_ICON[profile.type]} {typeLabel(profile.type, t)}</div>}
      <div style={{ position:'absolute', top:6, right:6, background:'rgba(10,20,26,.85)', borderRadius:10, padding:'2px 6px', fontSize:9, color:C.text }}>{profile.betweenScore}</div>
    </div>
    <div style={{ padding:'8px 8px 10px' }}>
      ...
      <div style={{ fontSize:10, color:C.muted, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{[profile.city, profile.distance && `${profile.distance} km`].filter(Boolean).join(' · ')}</div>
    </div>
  </button>
}
  ...
  return <>
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:20, overflow:'hidden', marginBottom:16 }}>
      <div style={{ height:260, background:`linear-gradient(160deg, ${C.elevated}, ${C.bg})`, position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
        {profile.hasPhotos && profile.primaryPhoto ? <img src={profile.primaryPhoto} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/> : <div style={{ width:72, height:72, borderRadius:'50%', background:C.elevated, border:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:28, color:C.muted }}>{TYPE_ICON[profile.type] || '○'}</div>}
        <div style={{ position:'absolute', top:12, left:12, background:'rgba(10,20,26,.85)', border:`1px solid ${C.border}`, borderRadius:20, padding:'4px 10px', fontSize:11, color:C.text }}>{profile.score} {t('explore.score')}</div>
    ...
    {matched && <div onClick={() => setMatched(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.92)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:16, padding:32 }}><div style={{ fontSize:56 }}>💫</div><div style={{ fontSize:28, fontWeight:500, color:C.primary, textAlign:'center' }}>{t('explore.newMatch')}</div><p style={{ color:C.text2, textAlign:'center' }}>{t('explore.matchHelp')}</p><button style={{ background:C.primary, border:'none', borderRadius:50, padding:'13px 32px', color:'#0A141A' }}>{t('explore.viewMatches')}</button></div>}
  </>
}

export default function ExploreScreen() {
  const { language } = useI18n()
  const t = createExploreTranslator(language)
  ...
  return <div className="app-screen app-screen--explore" style={{ padding:'calc(16px + env(safe-area-inset-top)) 16px 0', maxWidth:480, margin:'0 auto' }}>
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}><span style={{ fontSize:18, fontWeight:500, color:C.text }}>{t('explore.title')}</span><button onClick={load} aria-label={t('explore.refresh')} style={{ color:C.muted, fontSize:20, padding:8, minWidth:44, minHeight:44, borderRadius:10, border:`1px solid ${C.border}` }}>↻</button></div>
    ...
    {!loading && !error && <div className="explore-profile-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:10, paddingBottom:24 }}>{profiles.map(profile => <GridTile key={profile.id} profile={profile} onOpen={setSelected} t={t}/>)}</div>}
